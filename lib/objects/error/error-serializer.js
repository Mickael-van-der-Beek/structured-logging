  ...
  return genericSerializer(
    (err) => {
      ...
      if (serializedError.message) {
        const shortMessage = serializedError.message.slice(0, 256);

        if (serializedError.stack) {
          serializedError.stack = serializedError.stack.replace(serializedError.message, shortMessage);
        }

        serializedError.message = shortMessage;
      }

      if (serializedError.stack) {
        serializedError.stack = serializedError.stack.slice(0, 2048);
      }
