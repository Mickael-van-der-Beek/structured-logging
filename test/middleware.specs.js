...
describe('Express middleware', function () {
  ...
  it('don\'t override `req.logger` if one already exists and options.override is `false`', function (done) {
    var hostname = '127.0.0.1';
    var pathname = '/';
    var method = 'get';
    var port = 10000;
  ...
  it('does override `req.logger` if one already exists and options.override is `true`', function (done) {
    var hostname = '127.0.0.1';
    var pathname = '/';
    var method = 'get';
    var port = 10000;
      ...
      supertest(app)[method](pathname)
        .end(function (err, res) {
          ...
          process.nextTick(function () {
            assert.equal(stream1.write.calledOnce, true);
            stream1.write.restore();

            assert.equal(stream2.write.calledOnce, true);
            stream2.write.restore();
